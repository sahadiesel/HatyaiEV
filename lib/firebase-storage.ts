...
export const COMPANY_SIGNATURE_STORAGE_PATH = "Company/signature.png";
export const COMPANY_STAMP_STORAGE_PATH = "Company/stamp.png";

export const MAX_COMPANY_ASSET_BYTES = 1024 * 1024; // 1 MB

/**
 * อัปโหลดลายเซ็นหรือตรายางของบริษัทไป Firebase Storage
 * ถ้ารูปเกิน 1 MB ระบบจะย่อให้อัตโนมัติก่อนอัปโหลด
 */
export async function uploadCompanyAsset(
  kind: "signature" | "stamp",
  file: File,
): Promise<{ storagePath: string; downloadUrl: string; wasCompressed: boolean }> {
  const auth = getFirebaseAuth();
  if (!auth?.currentUser) {
    throw new Error("ต้องเข้าสู่ระบบก่อนอัปโหลด");
  }
  const storage = getFirebaseStorageClient();
  if (!storage) {
    throw new Error("ยังไม่ได้ตั้งค่า Firebase Storage");
  ...
  const { file: ready, wasCompressed } = await compressImageIfNeeded(file, MAX_COMPANY_ASSET_BYTES);

  const ext = extFromFile(ready);
  const path =
    kind === "signature"
      ? `Company/signature.${ext === "jpg" ? "jpg" : ext === "webp" ? "webp" : "png"}`
      : `Company/stamp.${ext === "jpg" ? "jpg" : ext === "webp" ? "webp" : "png"}`;

  const storageRef = ref(storage, path);
  await uploadBytes(storageRef, ready, { contentType: ready.type || "image/png" });
  const downloadUrl = await getDownloadURL(storageRef);
  return { storagePath: path, downloadUrl, wasCompressed };
}
