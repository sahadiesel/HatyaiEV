...
const baseNav: { href: string; label: string; menuId: MenuId }[] = [
  ...
  { href: "/vehicles", label: "รถยนต์และต้นทุน", menuId: "vehicles" },
  { href: "/entities", label: "คู่ค้า", menuId: "entities" },
  { href: "/documents", label: "ศูนย์เอกสาร", menuId: "documents" },
  { href: "/cashbook", label: "สมุดเงินสด", menuId: "cashbook" },
];
    ...
    <div className="flex min-h-screen bg-slate-50">
      <aside className="sticky top-0 flex h-screen w-56 shrink-0 flex-col border-r border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-4 py-4">
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">หาดใหญ่ อี วี</p>
        </div>
        ...
        <div className="border-t border-slate-200 p-4">
          {profile && (
            <>
              <p className="truncate text-sm font-medium text-slate-900" title={profile.email || profile.nationalId}>
                {profile.name || "ผู้ใช้"}
              </p>
              {(profile.email || profile.nationalId) && (
                <p className="truncate text-xs text-slate-500" title={profile.email || profile.nationalId}>
                  {profile.email || profile.nationalId}
                </p>
              )}
            </>
