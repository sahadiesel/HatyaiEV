...
export async function saveCompanySettings(formData: FormData): Promise<SaveCompanySettingsResult> {
  const payload: CompanySettingsFull = {
    ...
    signatureUrl: String(formData.get("signatureUrl") ?? ""),
    signatureStoragePath: String(formData.get("signatureStoragePath") ?? ""),
    stampUrl: String(formData.get("stampUrl") ?? ""),
    stampStoragePath: String(formData.get("stampStoragePath") ?? ""),
  };
