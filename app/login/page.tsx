...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (raw.includes("invalid-credential") || raw.includes("user-not-found") || raw.includes("wrong-password")) {
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-4 py-10">
      ...
      <p className="mt-1 text-sm text-slate-600">ใช้อีเมลและรหัสผ่านที่ลงทะเบียนไว้</p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <label className="block text-sm">
          ...
          <input name="password" type="password" required minLength={6} autoComplete="current-password" className={inp} />
        </label>
