...
export function UsersDirectoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      const hay = [u.name, u.email, u.nationalId, u.phone, u.address, u.recoveryEmail ?? ""].join(" ").toLowerCase();
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <label className="block text-sm">
          ...
          <input
            ...
            placeholder="ค้นหา ชื่อ / อีเมล / โทร"
              ...
              {filtered.map((u) => {
                ...
                return (
                  <tr key={u.uid} className="border-b border-slate-100 align-top">
                    ...
                    <td className="px-3 py-3 text-slate-700">{u.email || u.nationalId || "—"}</td>
      ...
      {editing && (
        ...
        >
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-600">
              {editing.name} · {editing.email || editing.nationalId || "—"}
            </p>
